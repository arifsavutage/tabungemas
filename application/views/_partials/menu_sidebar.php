        ...
        <div id="sidebar-menu">
            ...
            <ul>
                ...
                $query_menu = $this->db->query("SELECT tb_user_menu_access.`id`, `role_id`, `menu_id`, tb_user_role.role_name, tb_user_menu.title, tb_user_menu.url, tb_user_menu.icon, tb_user_menu.parent_menu, tb_user_menu.is_active
                    ...
                    WHERE tb_user_role.role_name = '$role'");
                ...
                foreach ($query_menu->result_array() as $menus) {
                    ...
                    /**Sub Menu 1 */
                    $submenu1 = $this->db->query("SELECT `id`, `title`, `url`, `icon`, `parent_menu`, `is_active` FROM `tb_user_menu` WHERE `parent_menu`= $menus[menu_id]");
                    foreach ($submenu1->result_array() as $child1) {
                        if ($child1['url'] == '#') {
                            ...
                            echo '<ul class="list-unstyled">';
                            /**Sub Menu 2 */
                            $submenu2 = $this->db->query("SELECT `id`, `title`, `url`, `icon`, `parent_menu`, `is_active` FROM `tb_user_menu` WHERE `parent_menu`= $child1[id]");
